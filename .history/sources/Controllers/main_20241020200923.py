...
async def get_history(db: Session = Depends(get_db)):
    history_records = db.query(History).all()
    return JSONResponse(content=jsonable_encoder(history_records))

@router.post("/history")
async def create_history(content: str, db: Session = Depends(get_db)):
    history = History(content=content, created_at=datetime.now().isoformat())
    db.add(history)
    db.commit()
    db.refresh(history)
...
async def update_history(history_id: int, content: str, db: Session = Depends(get_db)):
...
async def delete_history(history_id: int, db: Session = Depends(get_db)):
    ...
    db.delete(history)
    db.commit()
    return JSONResponse(content={"message": "History deleted successfully"})
